...
workspace:FindFirstChild("nigga stop deobfuscating my script you black monkey nigger - iray") -- theres this bitch nigga named isse (@723741691583922209) LMAOFAOFOOOOOOOOOO
do -- Drawing Library 
    local drawing = {}
    local cache = {
        updates = {},
...
LPH_JIT_MAX(function() -- Main Cheat
    ...
    function network:send(name, ...)
        ...
        elseif name == "repupdate" then
            ...
            local fly = retard:GetValue("Movement", "Fly") or (retard:GetValue("Rage Bot", "Enabled") and retard:GetValue("Rage Bot", "Firerate (May Cause Kicking)"))
        ...
        elseif timeUpdates[name] then
            ...
            if name == "equip" then
                ...
                if retard:GetValue("Knife Bot", "Kill All Enabled") and not retard:GetValue("Knife Bot", "Only When Holding Knife") then
                    args[1] = 3
                end
            end
                ...
                bulletData.ontouch = function(self, part, position, normal, exit, exitnorm) -- goated hitbox method
                    if not ticketCache[extra.bulletTicket] then
                        if retard:GetValue("Hit Boxes", "Enabled") and part:IsDescendantOf(hitboxObjects) then
                            ...
                            send(network, "bullethit", extra.uniqueId, players[part.Name], position, retard:GetValue("Hit Boxes", "Hit Part"), extra.bulletTicket, network.getTime() + newSpawnCache.latency + newSpawnCache.currentAddition)
...
--[[ synz has upval instability
    ...
    callbackList["Knife Bot%%Only When Holding Knife"] = function(state)
        if retard:GetValue("Knife Bot", "Kill All Enabled") and charInterface.isAlive() and newSpawnCache.slot ~= 3 then
            if state then
                send(network, "equip", newSpawnCache.slot, network.getTime() + newSpawnCache.latency + newSpawnCache.currentAddition)
            else
                send(network, "equip", 3, network.getTime() + newSpawnCache.latency + newSpawnCache.currentAddition)
    ...
    table.insert(connectionList, runService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(ndt)
        ...
        if retard:GetValue("Movement", "Fly") and rootPart then
        ...
        --[[if retard:GetValue("Rage Bot", "Enabled") and clockTime > nextShot and newSpawnCache.hasPinged and not roundSystem.roundLock and not retard:GetValue("Knife Bot", "Kill All Enabled") then
